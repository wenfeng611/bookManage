var title = "科天云全渠道客服系统" ;
var socketIM  , newuser = [] , newmessage = [];
newuser['mp3'] = '/images/new.mp3'; 
newmessage['mp3'] = '/images/message.mp3';
$(document).ready(function(){
    socketIM = io(schema+'://'+hostname+':'+port+'/im/agent?orgi='+orgi+"&userid="+userid,{multiplex :false} );
    socketIM.on('connect',function() {
		console.log("连接初始化成功");
		//请求服务端记录 当前用户在线事件
    ...
    socketIM.on('chatevent', function(data) {
        ...
        // console.log(data);
        var s = "服务中的人数："+data.users+"人，当前排队人数："+data.inQuene+"人，在线坐席数："+data.agents+"人";
        // console.log(s)
    	$('#agents_status').html(s);
...
var WebIM = {
	sendMessage:function(message , userid , appid , session , orgi , touser , agentstatus){
		// console.log('send message in webIm');
		// console.log(socketIM)
        socketIM.emit('message', {
			appid : appid ,
			userid:userid,
			sign:session,
			touser:touser,
